...
import styles from './styles/LaserFlowCard.module.scss';
...
const LaserFlow = ({
  ...
  useEffect(() => {
    ...
    // Handle mouse events on the parent container
    const parentContainer = mount.parentElement;
    const onMove = ev => updateMouse(ev.clientX, ev.clientY);
    const onLeave = () => mouseTarget.set(0, 0);

    if (parentContainer) {
      parentContainer.addEventListener('pointermove', onMove, { passive: true });
      parentContainer.addEventListener('pointerdown', onMove, { passive: true });
      parentContainer.addEventListener('pointerenter', onMove, { passive: true });
      parentContainer.addEventListener('pointerleave', onLeave, { passive: true });
    }
    ...
    return () => {
      ...
      if (parentContainer) {
        parentContainer.removeEventListener('pointermove', onMove);
        parentContainer.removeEventListener('pointerdown', onMove);
        parentContainer.removeEventListener('pointerenter', onMove);
        parentContainer.removeEventListener('pointerleave', onLeave);
      }
      geometry.dispose();
      material.dispose();
      renderer.dispose();
  ...
  return <div ref={mountRef} className={`${styles.laserFlow} ${className}`} />;
};

// Card component for application records
export const LaserFlowCard = ({
  ...
  wispSpeed = 10,
  wispIntensity = 12,
  verticalSizing = 40, // Increased from 20 to make beam taller
  ...
  const [laserOffset] = useState(0.0)
  ...
  const laserColor = themeColors[theme] || themeColors.cyan;
  ...
  useEffect(() => {
    // Instead of trying to manipulate shader coordinates, 
    // we'll use CSS transforms to position the laser overlay
    const alignLaser = () => {
      const card = cardRef.current;
      const metricBox = card?.querySelector('[data-metric-box="true"]');
      const laserOverlay = card?.querySelector(`.${styles.laserOverlay}`);

      if (card && metricBox && laserOverlay) {
        const cardRect = card.getBoundingClientRect();
        const metricRect = metricBox.getBoundingClientRect();
    ...
    alignLaser();
    const timer = setTimeout(alignLaser, 100);
    window.addEventListener('resize', alignLaser);

    return () => {
      clearTimeout(timer);
      window.removeEventListener('resize', alignLaser);
    };
  }, [displayTitle, displayDescription, displayMetricValue, displayMetricLabel]);
  ...
  return (
    ...
    >
      ...
      <LaserFlow
        ...
        verticalBeamOffset={laserOffset}
      />
